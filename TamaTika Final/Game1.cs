...
namespace GameBelajar
{
    public partial class Game3 : Form
    {
        ...
        private void DisplayCountingObjects()
        {
            ...
            try
            {
                using (var tempStream = new FileStream(selectedImage, FileMode.Open, FileAccess.Read))
                {
                    originalImage = Image.FromStream(tempStream);
                }
            }
            catch (Exception ex)
            {
            ...
            for (int i = 0; i < correctAnswer; i++)
            {
                PictureBox pb = new PictureBox
                {
                    ...
                    Location = new Point(IMAGE_MARGIN + i * IMAGE_SPACING, IMAGE_MARGIN)
                };
                pnlImages.Controls.Add(pb);
            }

            originalImage.Dispose(); // untuk menghindari memory leak
        }
        ...
        {
            if (sender is Button clickedButton && int.TryParse(clickedButton.Text, out int selectedAnswer))
            {
                ProcessAnswer(selectedAnswer == correctAnswer); 
            }
        }
